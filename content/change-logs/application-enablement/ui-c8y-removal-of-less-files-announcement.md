...
---
The {{< product-c8y-iot >}} Web SDK styles in the `@c8y/style` package have been migrated from LESS to SCSS. As a follow-up, the redundant `.less` files will be **removed** from `@c8y/style`. This change is currently available in the Continuous Deployment (CD) version and will ship in an upcoming maintenance version of the 2026 release. The exact release version will be announced separately. 
