...
4. If you have installed {{< product-c8y-iot >}} DataHub in the Edge 10.17 appliance, follow the instructions in [Setting up {{< product-c8y-iot >}} DataHub Edge](/edge/edge-working-with-datahub/#setting-up-datahub-edge) to install the same in Edge 10.18. Before you proceed with installing {{< product-c8y-iot >}} DataHub, change the ownership of the data lake folders using the following command:
   ```shell
