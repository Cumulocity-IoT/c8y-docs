...
```bash
ssh <LOGIN_USERNAME>@<DNS_NAME_OF_SERVER> -t "/bin/bash -c 'cd /tmp && sudo /opt/c8y/utilities/install_signed_package.sh /tmp/pulsar-edge-install-1018.0.0.tgz"
```
...
For more information on installing and troubleshooting the Messaging Service, see the Messaging Service installation & operations documentation] available on the [{{< c8y-resources-server >}}]({{< c8y-resources-server-link >}}) (credentials required).
