...
The hardware requirements for running a bare {{< product-c8y-iot >}} Edge instance are described in [Requirements](https://cumulocity.com/docs/2025/edge/edge-installation/#prerequisites). When {{< product-c8y-iot >}} DataHub Edge is additionally running, the hardware requirements of the virtual machine are as follows:
...
{{< c8y-admon-req >}}
For JDBC/ODBC you must configure {{< product-c8y-iot >}} Edge so that port 32010 can be accessed from the host system. For instructions on port forwarding see [Installing Edge](https://cumulocity.com/docs/2025/edge/edge-installation/).
{{< /c8y-admon-req >}}
