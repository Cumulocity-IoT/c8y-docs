...
- `domain` | string | Required <br/>
  ...
  <br/>If you used **c8yedge** tool to install, you can configure this field using the below command: 
  ```shell
...
- `company` | string | Required <br/>
  ...
  <br/>**Info**: This value is used only during the Edge installation and can’t be changed for existing installations.  
  All subsequent tenant changes must be made via the [user interface](/enterprise-tenant/managing-tenants/#to-view-or-edit-subtenant-properties) or the Cumulocity API.
  <br/>If you used **c8yedge** tool to install, you can configure this field using the below command: 
  ```shell
...
- `email` | string | Required <br/>
  ...
  <br/>**Info**: This value is used only during the initial Edge installation to bootstrap the admin account and cannot be changed for existing installations. All subsequent admin user changes must be made via the [user interface](/standard-tenant/managing-users/#to-edit-a-user) or the Cumulocity API.
  <br/>If you used **c8yedge** tool to install, you can configure this field using the below command: 
  ```shell
...
- `cumulocityPasswordSecretName` | string | Required <br/>
  ...
  <br/>**Info**: This value is used only during the Edge installation and can’t be changed for existing installations. All subsequent password changes must be made via the [user interface](/standard-tenant/managing-users/#to-edit-a-user) or the Cumulocity API.  
  <br/>**Info**: The Edge operator retrieves this secret from the **EDGE-CR-NAMESPACE**. Ensure that this secret is created before initiating the Edge deployment process.  
  <br/>**Important**: The password must be at least 8 letters long.
...
- `tlsSecretName` | string | Optional <br/>
  ...
  <br/>**Info**: The Edge operator retrieves this secret from the **EDGE-CR-NAMESPACE**. Ensure that this secret is created before initiating the Edge deployment or update process.
  <br/>If you used **c8yedge** tool to install, you can configure this field using the below command: 
  ```shell
...
- `tlsSecretName` | string | Optional <br/>
  ...
  <br/>**Info**: You can also reuse the secret name provided in the `spec.tlsSecretName` provided that the TLS/SSL certificate it references is issued by an intermediate Certificate Authority (CA) within your organization and can be added to the trusted certificate list of your Cumulocity cloud tenant.  
  <br/>**Info**: The Edge operator retrieves this secret from the **EDGE-CR-NAMESPACE**. Ensure that this secret is created before initiating the Edge deployment or update process.  
  <br/>See [Registering Edge in the cloud tenant](/edge/connecting-edge-to-cloud/#register-edge-on-cloud) for more details.
  <br/>If you used **c8yedge** tool to install, you can configure this field using the below command: 
  ```shell
...
- `resources` | [PodResources](#podresources) | Optional <br/>
  ...
  <br/>**Info**: Once Edge is installed, you can only increase this value, but cannot reduce.
  <br/>If you used **c8yedge** tool to install, you can configure this field using the below command: 
  ```shell
