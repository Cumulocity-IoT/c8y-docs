...
-   receive input from each device within a range of devices and send output to the trigger device or an asset. Note that asset output can only be used for sending cross-device aggregates.
    ...
    When a model uses an asset group as input, it acts on:
...
When a template parameter is used for an output block, then if the parameter's value is a range of devices or asset, then this is treated the same as if it were set to the trigger device or asset. The output goes to whichever device or asset triggered the model's evaluation, with each device within a range being treated independently. Typically, the same template parameter will be used for both input and output, so these will refer to the same range, and each device or asset is processed independently.
...
If you want to change this default value \(to show either more or less items\), you must change the tenant options. That is, you must send a `POST /tenant/options` request. For detailed information, see the information on the [tenant options](https://{{< domain-c8y >}}/api/core/#tag/Options) in the {{< openapi >}}.
