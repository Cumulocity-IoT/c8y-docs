...
#### MQTT version 5.0 behaviour {#core-mqtt-mqtt-5-behaviour}
...
Handling of Core MQTT topics is **identical** for both protocol versions. None of the additional [MQTT version 5.0 features](#mqtt-50-features) described elsewhere in this section are honoured on Core MQTT topics, namely:

* MQTT version 5.0 reason codes on `PUBACK`, `SUBACK` or `DISCONNECT` packets, described in [Error reporting](#mqtt-error-reporting), are not used to report errors related to processing on Core MQTT topics. Errors are reported only using the existing error reporting mechanism when using Core MQTT topics:
  * On the `s/e` topic for SmartREST.
  * On the `error` topic for [JSON via MQTT](/smartrest/json-via-mqtt/#error-handling).
* Client disconnect behaviour is identical for both protocol versions. Regardless of whether a client is using MQTT version 3.1.1 or MQTT version 5.0, the disconnect behaviour on Core MQTT topics is exactly the same, for example when sending an invalid message or in the case of a server processing error.
...
{{< c8y-admon-caution >}}
Devices using MQTT version 5.0 that are using Core MQTT topics must not rely on any MQTT version 5.0 feature being honoured on those topics.
{{< /c8y-admon-caution >}}
