...
To access it, you must first install the "Dashboard details advanced tab" plugin to your Cockpit application. This is a plugin with the scope `self-optional`, so it has to be installed manually. Then navigate to the dashboard settings and open the **Import/export** tab.
