...
{{< c8y-admon-info >}}
This section describes how to manage trusted certificates. For information on connecting devices using certificates refer to [Trusted certificates](/device-certificate-authentication/device-certificates/).
{{< /c8y-admon-info >}}
...
| Auto registration | If selected, new devices that use a certificate signed by the authority owning this trusted certificate will be automatically registered. The option does not support devices using the LWM2M protocol. If deselected, devices that do not already exist in the system will not automatically be registered, and the connection will be rejected; such devices must be pre-registered using the full-bulk registration process.      |
