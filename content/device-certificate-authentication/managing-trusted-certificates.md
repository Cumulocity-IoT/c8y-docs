...
| Auto registration | If selected, new devices which use a certificate signed by the authority owning this trusted certificate will automatically be registered. The option does not support devices using the LWM2M protocol. If deselected, devices that do not already exist in the system will not be automatically registered and connection will be rejected; such devices must be pre-registered using the full-bulk registration process.      |
