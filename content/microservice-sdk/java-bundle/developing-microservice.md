...
(If a single xml tag is specified as parameter in the following list, use embracing xml tags like \<tag>...\</tag> to set those parameters in pom.xml. "..." must be replaced by the respective value of the corresponding data type. Use "Parameter command line name" to set the corresponding parameter on maven command line.)
