...
* **Device settings** - General device parameters:
  ...
  * Automatic setting of required interval (When enabled, the LWM2M agent automatically sets the interval to registration lifetime plus 2 minutes. When false, the user can define a required interval for the device on the Info tab in the device details. If value is not provided ("null"), the default behavior of the LWM2M service will be used.)
