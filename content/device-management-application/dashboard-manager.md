...
{{< c8y-admon-info >}}
Dashboards are application-specific. A dashboard created within Device Management is unique to that specific instance; it will not appear in other applications like Cockpit or renamed cloned versions of the Device Management application.
{{</ c8y-admon-info >}}
...
On enabling the template option for the dashboard, the dashboard will become available to all the devices with the same type. These shared dashboards are marked with a 'T' icon, making them easy to distinguish from device-specific views.
