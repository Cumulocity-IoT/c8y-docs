...
{{< c8y-admon-info >}}
When creating a new user, the owner is automatically set to the user who is logged in if the logged-in user has only "User management" CREATE permission. The owner can be changed later by a user with 'User management' ADMIN permission, or by a user with 'User management' CREATE permission if they are the current owner (parent) of the useer account.
