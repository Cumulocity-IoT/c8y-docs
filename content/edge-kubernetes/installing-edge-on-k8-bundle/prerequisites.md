...
|Disk space|100 GB </p><p><p>If the {{< product-c8y-iot >}} Messaging Service is enabled, which is required for enabling the microservice-based data broker and Notifications 2.0, an additional **15 GB** for Pulsar's persistent message storage. </p><br>For more information about configuring the storage, see [Configuring storage](/edge-kubernetes/installing-edge-on-k8/#configuring-storage).|
