...
|Disk space|100 GB <p>An additional **15 GB** is required for Pulsar’s persistent message storage if the {{< product-c8y-iot >}} Messaging Service is enabled, which is required for the microservice-based data broker and Notifications 2.0. <p>For more information about configuring the storage, see [Configuring storage](/edge-kubernetes/installing-edge-on-k8/#configuring-storage).|
