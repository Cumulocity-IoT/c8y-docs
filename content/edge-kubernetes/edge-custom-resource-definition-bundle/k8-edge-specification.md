...
| <div style="width:300px"> Field </div>| Description | <div style="white-space: nowrap"> CLI  </div>| Default | Validation |
| --- | --- | --- | --- | --- |
...
| Field | Description | CLI | Default | Validation |
| --- | --- | --- | --- | --- |
| `name` _string_ | Specify the name of the microservice to configure. For example, 'apama-ctrl'.<br /> | `c8yedge config --set microservices.<microservice-name>.resources.limits.cpu=value` |  |  |
...
| `resources` _[PodResources](#podresources)_ | Specify resource limits for the MongoDB server. Specify the size of the Persistent Volume Claim (PVC) named mongod-data-edge-db-rs0-0 made by MongoDB server for persisting application data.``<br /> | `c8yedge config --set mongodb.resources.limits.cpu=value --set mongodb.resources.limits.memory=value --set mongodb.resources.requests.storage=value` |  |  |
