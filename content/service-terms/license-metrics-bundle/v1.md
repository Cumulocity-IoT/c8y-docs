...
A data transaction is defined as any authenticated API request with the intention to create, update, or process data within the {{< product-c8y-iot >}} domain model, including inventory objects, operations, measurements, events, and alarms. Transactions are counted regardless of processing mode, the details of which can be found in the [Technical concepts section of the {{< product-c8y-iot >}} documentation](https://{{< domain-c8y >}}/docs/concepts/realtime/#implementation). Transactions originate from both internal and external sources, including other {{< product-c8y-iot >}} applications, and are recorded via the platform’s [Usage Statistics API](https://{{< domain-c8y >}}/api/core/#tag/Usage-statistics).
...
All counted transactions and messages are subject to the quotes listed in [Service quotas](https://{{< domain-c8y >}}/docs/service-terms/quotas/). Usage that exceeds the listed limits may result in additional charges to ensure platform stability and prevent abuse.
