...
* **eMail notification**:  {{< company-c8y >}} provides basic notification service via standard SMTP (unencrypted, port 25) for their customers to use via {{< company-c8y >}} infrastructure components (SMTP relay). This infrastructure has been setup to transfer emails using {{< company-c8y >}} domain, including e.g. correct SPF records for this domain. Customers using an enterprise tenant can configure their own eMail domain and relay and use e.g. encrypted SMTP (STARTTLS) or SMTPS with their own SMTP relay. If customer uses their own email domain, it is responsibiltiy of the customer to configure e.g. correct SPF records for that domain.
