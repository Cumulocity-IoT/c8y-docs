...
### I see a device in `latest_inventory` but not in `cdc_inventory`. Is this expected? {#latest-inventory-not-in-cdc}
...
### My device, alarm … appears several times in the inventory in the lake. {#duplicate-records-in-lake}
