...
#### {{< product-c8y-iot >}} DataHub Edge variants {#datahub-edge-variants}
...
| -----   | -----   | -----   | -----   |
| High Availability | Depending on the underlying virtualization technology | Depending on the underlying virtualization technology | Depending on the cloud deployment setup |
| Vertical scalability | Yes | Yes | Yes |
| Horizontal scalability | No | No | Yes |
| Upgrades with no downtime | No | No | No |
| Root access | No | No | Yes, if customer is hosting |
| Installation | Offline | Offline & Online | Online |
