...
The RBAC model is implemented and managed through the User API. Endpoints under `/user/roles` and `/user/inventoryroles` are used to define roles, while endpoints like `/user/users/{username}/roles` assign them to users.
{{< /c8y-details >}}  
...
{{< c8y-details title="Developer details" >}}
Roles are managed via the User API. Global roles are managed via `/user/roles`, while inventory roles are managed via `/user/inventoryroles`.
