...
Users are managed via the User API (`/user/users`). This includes creating (POST), retrieving (GET), updating (PUT), and deleting (DELETE) users. Role assignments are managed via sub-resources like `/user/users/{username}/roles`.
{{< /c8y-details >}}
...
{{< c8y-details title="Developer details" >}}
User hierarchies are managed via the User API (`/user/users`). Creating/updating a user with an owner property establishes the hierarchy.
{{< /c8y-details >}}  
