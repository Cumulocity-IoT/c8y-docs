...
{{< c8y-details title="Developer details" >}}
Groups are represented as managed objects (`/inventory/managedObjects`), using a `c8y_IsDeviceGroup` fragment and often with type `c8y_DeviceGroup`. Assets, devices or subgroups are assigned via child asset references (`POST /inventory/managedObjects/{groupId}/childAssets`).
{{< /c8y-details >}}
